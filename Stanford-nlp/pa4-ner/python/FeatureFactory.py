...
class FeatureFactory:
    ...
    def computeFeatures(self, words, previousLabel, position):
        ...
        if currentWord.istitle() :
            features.append("case1=Title1")

        if not currentWord.isupper() :
            features.append("case2=Title2")

        if currentWord.isalpha() :
            features.append("case3=Title3")

        if prevWord.isdigit():
            features.append("indigit=digit")

        if "'" in currentWord:
          features.append("accent=accent")

        if previousLabel == "PERSON" or len(prevWord) < 3:
          features.append("prev=Person")

        if prevWord == "'" or prevWord == '"' or prevPrevWord == "'" or prevPrevWord == '"':
            features.append("inQ=inq")

        if prevWord == "at" or prevPrevWord == "at":
          features.append("isPlace=place")
